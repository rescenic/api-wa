...
var { Nulis, Vokal, Base, Searchnabi, Gempa } = require("./../lib");
var cookie =
...
router.get("/randomquote", async (req, res, next) => {
    ...
    fetch(
        encodeURI(`https://python-api-zhirrr.herokuapp.com/api/random/quotes`)
    )
...
router.get("/nulis", async (req, res, next) => {
    ...
    Nulis(text)
        .then((hasil) => {
            fetch(
                encodeURI(
                    `https://api.imgbb.com/1/upload?expiration=120&key=761ea2d5575581057a799d14e9c78e28&image=${hasil}&name=${randomTextNumber}`
                )
            )
                .then((response) => response.json())
                .then((data) => {
...
router.get("/cuaca", async (req, res, next) => {
    ...
    if (!kabupaten)
        return res.json({
            status: false,
            creator: `${creator}`,
            message: "masukan parameter kabupaten",
        });
    fetch(
        encodeURI(
            `https://docs-api-zahirrr.herokuapp.com/api/cuaca?kabupaten=${kabupaten}`
        )
    )
        .then((response) => response.json())
        .then((data) => {
